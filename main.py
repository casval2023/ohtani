...
def calculate_discriminant_score(coefficients, release_speed,plate_x,plate_z,pfx_x,pfx_z):
    score = (coefficients["release_speed"] * release_speed + coefficients["plate_x"] * plate_x + coefficients["plate_z"] * plate_z + coefficients["pfx_x"] * pfx_x + coefficients["pfx_z"] * pfx_z + coefficients["constant"])
...
st.write("※ヒットを打たれない確率なので、ファウルや見逃し、空振り等、ヒット以外全てを含みます")

# 入力部分
strike = st.number_input("ストライクカウント", 0, 2, 1)
speed = st.number_input("ボールの速度 (km/h)", 100.0, 180.0, 145.0)
zone = st.selectbox("コース", ["内角高め", "真ん中高め", "外角高め", "内角真ん中", "ど真ん中", "外角真ん中", "内角低め", "真ん中低め", "外角低め"])
